...
static bool callEventListener(const std::string& eventName, JSCValue* event)
{
    ...
    JSCValue* result = jsc_value_function_call(it->second, JSC_TYPE_VALUE, event, G_TYPE_NONE);
    if (result)
...
static gboolean onUserMessageReceived(WebKitWebPage* page, WebKitUserMessage* message)
{
...
static void onPageCreated(WebKitWebExtension* webExtension, WebKitWebPage* page, gpointer userData)
{
...
extern "C"
{
    ...
    {
        (void) extension;
        g_info("Initialize TestRunner extension");
