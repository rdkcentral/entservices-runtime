...
public:
    ...
    bool EnsureInitialized(WebKitWebView* parent_view, const std::string& extension_dir);
...
bool TestRunnerImpl::EnsureInitialized(WebKitWebView* parent_view, const std::string& extension_dir) {
    if (m_parentView)
        return true;
...
std::unique_ptr<TestRunner> TestRunner::Create(WebKitWebView* parent, const std::string& extensionDir) {
    std::unique_ptr<TestRunnerImpl> testRunner = std::make_unique<TestRunnerImpl>();
    if (testRunner->EnsureInitialized(parent, extensionDir))
        return testRunner;
    return {};
}
